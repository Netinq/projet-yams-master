...
const updateClientsTickets = (game) => {
  setTimeout(() => {
    game.player1Socket.emit('game.tickets', GameService.send.forPlayer.gameTickets('player:1', game.gameState));
    game.player2Socket.emit('game.tickets', GameService.send.forPlayer.gameTickets('player:2', game.gameState));
  }, 200);
}

const updateClientsResumeData = (game) => {
  setTimeout(() => {
    game.player1Socket.emit('game.resumeData', GameService.send.forPlayer.gameResumeData('player:1', game.gameState));
    game.player2Socket.emit('game.resumeData', GameService.send.forPlayer.gameResumeData('player:2', game.gameState));
  }
  , 200);
}
...
const createGame = (player1Socket, player2Socket) => {
  ...
  // calculate score for testing
  games[gameIndex].gameState.player1.score = GameService.score.calculatePlayerScore('player:1', games[gameIndex].gameState);
  games[gameIndex].gameState.player2.score = GameService.score.calculatePlayerScore('player:2', games[gameIndex].gameState);

  // we update views
  updateClientsViewTimers(games[gameIndex]);
  updateClientsViewDecks(games[gameIndex]);
  updateClientsViewGrid(games[gameIndex]);
  updateClientsScore(games[gameIndex]);
  updateClientsTickets(games[gameIndex]);
  ...
  const gameInterval = setInterval(() => {
    ...
    if (games[gameIndex].gameState.timer === 0) {
      ...
      if ((games[gameIndex].gameState.currentTurn === 'player:1' && games[gameIndex].gameState.player1.tickets === 0)
      || (games[gameIndex].gameState.currentTurn === 'player:2' && games[gameIndex].gameState.player2.tickets === 0)) {
      games[gameIndex].gameState.currentTurn = games[gameIndex].gameState.currentTurn === 'player:1' ? 'player:2' : 'player:1';
    }
  ...
  player1Socket.on('game.end', () => {
    clearInterval(gameInterval);
    updateClientsResumeData(games[gameIndex]);
  });

  player2Socket.on('disconnect', () => {
    clearInterval(gameInterval);
  });
  player2Socket.on('game.end', () => {
    clearInterval(gameInterval);
    updateClientsResumeData(games[gameIndex]);
  });
...
io.on('connection', socket => {
  ...
  socket.on('game.dices.roll', () => {
    ...
    games[gameIndex].gameState.grid = GameService.grid.resetcanBeCheckedCells(games[gameIndex].gameState.grid);
    games[gameIndex].gameState.grid = GameService.grid.updateGridWithChoice(combinations, games[gameIndex].gameState.grid);
  ...
  socket.on('game.grid.selected', (data) => {
    ...
    if (games[gameIndex].gameState.currentTurn === 'player:1') games[gameIndex].gameState.player1.tickets -= 1;
    if (games[gameIndex].gameState.currentTurn === 'player:2') games[gameIndex].gameState.player2.tickets -= 1;

    console.log('player1 tickets', games[gameIndex].gameState.player1.tickets);
    console.log('player2 tickets', games[gameIndex].gameState.player2.tickets);

    if (GameService.game.checkForEndGame(games[gameIndex].gameState)) {
